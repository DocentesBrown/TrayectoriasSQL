...
function backendUrl() {
  // 1) Primero: URL guardada en este navegador (no requiere editar archivos)
  const override = localStorage.getItem('TRAYECTORIAS_BACKEND_URL_OVERRIDE') || '';
  if (override && override.includes('/exec')) return override;

  // 2) Fallback: config.js
  const u = window.TRAYECTORIAS_BACKEND_URL;
  if (u && !u.includes('PEGAR_WEB_APP_URL_AQUI')) return u;

  throw new Error(
    'Falta configurar la URL del backend. En la pantalla de ingreso, abrí “Configurar backend” y pegá la URL que termina en /exec.'
  );
}
...
// Guardar URL del backend (Apps Script) en localStorage para no editar archivos
if ($('btnSaveBackendUrl')) {
  $('btnSaveBackendUrl').onclick = async () => {
    const input = $('backendUrlInput');
    const msg = $('backendUrlMsg');
    const url = (input ? String(input.value || '').trim() : '');

    if (!url || !url.includes('/exec')) {
      setMessage('backendUrlMsg', 'Pegá una URL válida que termine en /exec', 'err');
      return;
    }

    localStorage.setItem('TRAYECTORIAS_BACKEND_URL_OVERRIDE', url);
    setMessage('backendUrlMsg', 'URL guardada ✅', 'ok');

    // Test rápido
    try {
      await apiCall('ping', {});
      setMessage('backendUrlMsg', 'URL OK ✅ (ping)', 'ok');
    } catch (e) {
      setMessage('backendUrlMsg', 'Guardé la URL, pero el ping falló: ' + (e?.message || e), 'err');
    }
  };
}
